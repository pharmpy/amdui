...
const bestGuess = (column: string): typeof kinds[keyof typeof kinds] => {
...
const ColumnKindSelector = ({column}: ColumnKindSelectorProps) => {
  const labelId = useId();
...
const DatasetColumnConfiguration = ({columns}: DatasetColumnConfigurationProps) => {
  return (
        ...
        <TableBody>
          {columns.map((column) => {
            return (
              <TableRow key={column}>
                <TableCell>{column}</TableCell>
                <TableCell><ColumnKindSelector column={column}/></TableCell>
              </TableRow>
            );
          })}
        </TableBody>
